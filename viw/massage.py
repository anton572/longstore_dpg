...
class Massage(BoxLayout):
    def __init__(self):
        super().__init__(padding=0,orientation='vertical',pos_hint={'x':0.,'y':0.},size_hint=(None,None),size=(120,120))
    def add_widget(self, text):
        if len(self.children)==6:
            self.remove_widget(self.children[-1])
        Layout=BoxLayout(padding=0,size_hint=(None,None),size=(120,30))
